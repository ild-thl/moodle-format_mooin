            ...
            foreach($bottom_button_other_data as $val) {
                ...
                if (!$this->page->user_is_editing()) {
                    if(get_user_preferences('section_progress_label-'.$user_pref) == '') {
                        ...
                        $bar .= html_writer::start_tag('button', array('type' => 'button', 'class'=>'comp_btn btn-outline-secondary btn_comp bottom_complete-' .$course->id, 'style'=>'margin-top: 30px', 'id' => 'id_bottom_complete-' .$val->id, 'name'=> 'btnComplete-' . $displaysection,'value' => 'Seite als bearbeitet markieren', )); // , 'type' => 'submit'
