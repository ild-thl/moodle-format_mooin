...
class testlib {
    ...
    */
    ...
    function get_section_prefix($section) {
        ...
        $parentchapter = $this->get_parent_chapter($section);
        if (is_object($parentchapter)) {
            $sids = $this->get_sectionids_for_chapter($parentchapter->id);
    ...
    function get_parent_chapter($section) {
        ...
        foreach ($chapters as $chapter) {
            $sids = $this->get_sectionids_for_chapter($chapter->id);
    ...
    function get_sectionids_for_chapter($chapterid) {
        ...
        if ($chapter = $DB->get_record('format_mooin4_chapter', array('id' => $chapterid))) {
            $chapters = $this->get_course_chapters($chapter->courseid);
            $start = 0;
            $end = 0;
            foreach ($chapters as $c) {
                if ($c->id == $chapterid) {
