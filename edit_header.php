...
$url = new moodle_url('/course/format/mooin/edit_header.php', array('course' => $courseid));
...
$PAGE->set_heading(get_string('edit_course_header', 'format_mooin'));
...
if ($mform->is_cancelled()) {
    ...
    if (isset($fromform->headerimagedesktop)) {
        ...
        throw new coding_exception(get_string('file_save_error', 'format_mooin'));
    }
    ...
    file_prepare_draft_area($draftitemiddesktop, $coursecontext->id, 'format_mooin', 'headerimagedesktop', $courseid, array('maxfiles' => 1));
    $draftitemidmobile = file_get_submitted_draft_itemid('headerimagemobile');
    file_prepare_draft_area($draftitemidmobile, $coursecontext->id, 'format_mooin', 'headerimagemobile', $courseid, array('maxfiles' => 1));
