...
class utils {
   ...
   public static function get_user_badge_progress($userid, $courseid) {
        ...
        foreach ($userbadges as $ub) {
            $badgeid = property_exists($ub, 'badgeid') ? $ub->badgeid : (property_exists($ub, 'id') ? $ub->id : null);
            if ($badgeid !== null) {
                $badgeMap[$badgeid] = [
                    'unlocked' => true,
                    'dateissued' => userdate($ub->dateissued ?? time())
                ];
            }
        }
        ...
        foreach ($coursebadges as $badge) {
            $unlocked = array_key_exists($badge->id, $badgeMap);
            $dateissued = $unlocked ? $badgeMap[$badge->id]['dateissued'] : null;
            $badgeobj = new \core_badges\badge($badge->id);
            $criteria = $badgeobj->get_criteria();

            $conditions = [];
            $required = 0;
            $completed = 0;
            $completedcriteriaids = array_map(function($c) {
                return $c->critid;
            }, $badgeobj->get_criteria_completions($userid));
            
            if ($badgeobj->has_criteria()) {
                foreach ($criteria as $type => $criterion) {
                    ...
                    if ($criterion instanceof \award_criteria_manual) {
                        foreach ($criterion->params as $subid => $subdata) {
                            $isdone = in_array($subid, $completedcriteriaids);
                            $roleid = $subdata['role'];
                            $role = $DB->get_record('role', ['id' => $roleid]);
                            $label = $role ? role_get_name($role, context_course::instance($courseid)) : get_string('manual_award_by_role', 'badges', $roleid);
                            $subconditions[] = [
                                    'label' => $label,
                                    'done' => $isdone
                                ];
                        ...
                        elseif ($criterion instanceof \award_criteria_activity) {
                            foreach ($criterion->params as $modid => $moddata) {
                                $cm = get_coursemodule_from_id(null, $modid);
                                $label = $cm ? $cm->name : get_string('activity', 'badges');
                                $done = false;
                                ...
                                if ($cm) {
                                    ...
                                    if ($completion->is_enabled($cm_info)) {
                                        $completiondata = $completion->get_data($cm_info, true);
                                        $done = $completiondata && $completiondata->completionstate == COMPLETION_COMPLETE;
                                    }
                                }
                                $subconditions[] = [
                                    'label' => $label,
                                ...
                                $localrequired++;
                                if ($done) $localcompleted++;
                            }
                        }
                        else {
                            $isdone = in_array($criterion->id, $completedcriteriaids);
                            $label = trim(strip_tags($criterion->get_details($userid)));
                            $label = $label ?: get_string('nocriteria', 'badges');
                        ...
                        usort($subconditions, function($a, $b) {
                            return ($a['done'] === $b['done']) ? 0 : ($a['done'] ? 1 : -1);
                        });

                        $conditions = array_merge($conditions, array_slice($subconditions, 0, 3));
                            if ($method == 2  && $localcompleted > 0) {
                                $required += 1;
                                $subconditions = array_filter($subconditions, fn($c) => $c['done']);
                                if ($localcompleted > 0) $completed += 1;
                            } elseif($method == 1) { 
            ...
            $badgeresult[] = [
                ...
                'overview_url' => new \moodle_url('/badges/overview.php', ['id' => $badge->id]),
                'unlocked' => $unlocked,
                'progress' => $completed,
                'required' => $required,
                'conditions' => $conditions,
                'progress_percent' => $required > 0 ? floor(($completed * 100) / $required) : 0,
