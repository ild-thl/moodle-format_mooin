...
$PAGE->set_url('/course/format/mooin4/forums.php', $params);
...
if ($cmid > 0) {
    ...
    if (!$forum = $DB->get_record('forum', ['id' => $cm->instance])) {
        throw new moodle_exception('forumnotfounderror');
    }
} else if ($id > 0) {
    if (!$forum = $DB->get_record('forum', ['course' => $id, 'type' => 'general'])) { // General.
        throw new moodle_exception('forumnotfounderror');
    }
    ...
    $forumcm = $DB->get_record('course_modules', [
        'instance' => $forum->id,
        'course' => $course->id,
        'module' => $forummod->id,
    ]);
...
if (!empty($CFG->enablerssfeeds) && !empty($CFG->forum_enablerssfeeds) && $forum->rsstype && $forum->rssarticles) {
    ...
    $rsstitle = format_string($course->shortname, true, ['context' => context_course::instance($course->id)]);
    $rsstitle .= ': ' . format_string($forum->name);
...
$mythreadsurl = new moodle_url('/mod/forum/user.php', ['id' => $USER->id, 'course' => $course->id]);
$advancedsearchurl = new moodle_url('/mod/forum/search.php', ['id' => $course->id]);
...
if ($markasread) {
    if ($discussions = $DB->get_records('forum_discussions', ['forum' => $forum->id])) {
...
if (forumlib::count_unread_posts($USER->id, $course->id, false, $forum->id) > 0) {
    ...
    echo html_writer::link(
        new moodle_url('/course/format/mooin4/forums.php?f=' . $forum->id . '&markasread=1'),
        get_string('mark_all_as_read', 'format_mooin4')
    );
    echo '</P>';
}
...
if ($forum->forcesubscribe == 0 || $forum->forcesubscribe == 2) {
    sesskey();
    $subscription = $DB->get_record('forum_subscriptions', ['userid' => $USER->id, 'forum' => $forum->id]);
    if ($subscription) {
        echo html_writer::link(
            new moodle_url('/mod/forum/subscribe.php?id=' . $forum->id . '&sesskey=' . $USER->sesskey),
            get_string('unsubscribe', 'forum')
        );
    } else {
        echo html_writer::link(
            new moodle_url('/mod/forum/subscribe.php?id=' . $forum->id . '&sesskey=' . $USER->sesskey),
            get_string('subscribe', 'forum')
        );
    }
    echo '<p></p>';
}
...
if ($forum->type == 'single') {
    $discussion = null;
    $discussions = $DB->get_records('forum_discussions', ['forum' => $forum->id], 'timemodified ASC');
